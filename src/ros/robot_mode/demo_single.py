...
with FlyTello(my_tellos, get_status=True) as fly:
    ...
    fixed_yaw = str(0) #fly.get_status("yaw", tello=1, sync=True) # only record this the very first takeoff
    print("fixed yaw: " + fixed_yaw)
    fly.takeoff()
    fly.up(dist=20)
    fly.forward(dist=80)    
    time.sleep(5)
    ...
    if backward:
        ...
        fly.straight_from_pad(x=-8, y=0, z=25, speed=100, pad='m4')
        time.sleep(4)       
        fly.straight_from_pad(x=-7, y=3, z=25, speed=100, pad='m4')
        time.sleep(2)
    
    land_yaw = fly.get_status("yaw", tello=1, sync=True)
    print("land yaw: " + land_yaw)
    diff_yaw = int(land_yaw) - int(fixed_yaw)
    if diff_yaw > 0:
